...
def dislegomena_ratio(words):  # per document only a float value
    ...
    VN = lambda i: freqs[i]
    return float(VN(2) * 1.0 / (vocabulary_size))
...
def syllable_count(word):
    try:
        syl = [
            ...
            for x in cmuDictionary[word.lower()]
        ][0]
    except:
...
def pos_freq(text, NoOfSentences):
    ...
    features_dict = {
        **{
            k: v / NoOfSentences
            for k, v in Counter([token.tag_ for token in doc]).items()
        },
        **{
            k: v / NoOfSentences
            for k, v in Counter([entity.label_ for entity in doc.ents]).items()
        },
    }
...
def create_stylometrics(authorship, max_sentence=200):
    ...
    for ff, id, author in tqdm(
        zip(authorship["file"], authorship["id"], authorship["author"]),
        total=len(authorship),
    ):
    ...
    stylo_df = (
        pd.DataFrame(stylo_dict)
        .transpose()
        .rename_axis(["author", "id"])
        .reset_index()
        .fillna(0)
    )
...
def build_authorship(dir):
    ...
    for author in authors:
        books = [
            (author, b, os.path.join(dir, author, b))
            for b in os.listdir(os.path.join(dir, author))
        ]
