...
def style_embedding_evaluation(
    embeddings, features, kernel="rbf", n_fold=10, output="agg"
):
    ...
    res_df = pd.DataFrame.from_dict(
        {
            "mean": res_df.groupby("family")["mse_test"].mean(),
            "std": res_df.groupby("family")["mse_test"].std(),
        },
        orient="columns",
    )
...
def multi_style_evaluation(embeddings, names, features, n_fold=10):
    ...
    result_dfs = []  # List to collect DataFrames for concatenation

    for embedding, name in zip(embeddings, names):
        print(f"Evaluating model : {name}\n", flush=True)
        res_df = style_embedding_evaluation(
            embedding, features, n_fold=n_fold, output="agg"
        ).T

        # Add mean and std rows to the collection
        result_dfs.append(res_df.loc[["mean"]].copy())
        result_dfs.append(res_df.loc[["std"]].copy())

    # Concatenate all results at once
    if result_dfs:
        full_df = pd.concat([full_df] + result_dfs, ignore_index=True)

    # Set the embedding names
...
def style_spyder_charts(
    df_results, title="MSE Score for Style Evaluation of Embeddings"
):
    # Filter for the mean rows (every other row starting from index 0)
    mean_rows = df_results.iloc[::2]

    N = 8
    theta = radar_factory(N, frame="polygon")

    spoke_labels = list(mean_rows.drop("Embedding", axis=1).columns)

    fig, ax = plt.subplots(figsize=(9, 9), subplot_kw=dict(projection="radar"))

    colors = ["b", "r", "g", "c", "m", "y", "k"][: len(mean_rows)]
    ...
    for d, color in zip(np.array(mean_rows.loc[:, spoke_labels]), colors):
        ax.plot(theta, d, color=color)
        ax.fill(theta, d, facecolor=color, alpha=0.25)
    ax.set_varlabels(spoke_labels)

    # add legend relative to top-left plot
    labels = tuple(mean_rows.Embedding)
    legend = ax.legend(labels, loc=(0.8, -0.1), labelspacing=0.1, fontsize="small")
